...
public class DAONhanVien extends UnicastRemoteObject implements IDAONhanVien {
    ...
    //get NV theo ten
    @Override
    public ArrayList<NhanVien> getNhanVienTheoTen(String keyword) {
        String sql = "SELECT * FROM NhanVien WHERE MATCH(tenNV) AGAINST(:keyword IN NATURAL LANGUAGE MODE)";
        Query query = em.createNativeQuery(sql, NhanVien.class);
