...
public class PnlBanVe extends JPanel implements ActionListener, KeyListener, ISeatUpdateListener, IUpdateChuyenTauManagerListener {
    private final NhanVien nhanVien;
    private ISeatUpdateListener stub;  // Thêm biến này
    private IUpdateChuyenTauManagerListener updateChuyenTauListener;
    ...
    private final IDAOVe daoVe;
    private final IDAOGa daoGa;
    private final IDAOLoaiToa daoLoaiToa;
    private final IDAOChuyenTau daoChuyenTau;
    private final IDAOChoNgoi daoChoNgoi;
    ...
    public PnlBanVe(NhanVien nhanVien) throws RemoteException {
        ...
        // Đăng ký listener cho sự kiện đóng cửa
        ...
        this.daoLoaiToa = RemoteLookup.getRemote("loaiToaDao", IDAOLoaiToa.class);
        this.daoChuyenTau = RemoteLookup.getRemote("chuyenTauDao", IDAOChuyenTau.class);
        this.daoChoNgoi = RemoteLookup.getRemote("choNgoiDao", IDAOChoNgoi.class);
        ...
        try {
            ...
            daoVe.registerSeatUpdateListener(nhanVien.getTaiKhoan().getMaTK(), this);
            daoChuyenTau.registerUpdateChuyenTauListener(nhanVien.getTaiKhoan().getMaTK(), this);
        ...
        pnlChuyenTau.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                if (chuyenDangChon != null) {
                    // System.out.println(chuyenDangChon+"Uaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
                    // Kiểm tra nếu 2 chuyến có khác chiều không
                    if (chuyenDangChon.getMaChuyen().equals(chuyenTau.getMaChuyen())) {
                        ...
                        registerSeatListenerForCurrentTrain();
    ...
    public JPanel getPnlChoNgoi(ChoNgoi choNgoi, int index) {
        ...
        // PHẦN CẦN CẬP NHẬT - BẮT ĐẦU
        // Kiểm tra chỗ ngồi này đã bán chưa
        boolean isDaBan = listVeDaBan.stream().anyMatch(v ->
                v.getChuyenTau().getMaChuyen().equals(chuyenDangChon.getMaChuyen()) &&
                        v.getChoNgoi().getMaCho().equals(choNgoi.getMaCho()));

        // Kiểm tra chỗ ngồi này phải là vé tạm không
        boolean isVeTam = getListVeTuHoaDonTam().stream().anyMatch(
                veTam -> veTam.getChuyenTau().getMaChuyen().equals(chuyenDangChon.getMaChuyen()) &&
                        veTam.getChoNgoi().getMaCho().equals(choNgoi.getMaCho()));
        // PHẦN CẦN CẬP NHẬT - KẾT THÚC
        ...
        setChoNgoiColor(pnlChoNgoi, trangThai);
    ...
    private void setChoNgoiColor(JPanel pnlChoNgoi, String trangThai) {
        ...
        } else if (trangThai.equals(ETrangThaiChoNgoi.DANG_CHON.getTrangThai())) {
            pnlChoNgoi.setBackground(EColor.GHE_DANG_CHON.getColor()); // Màu cam
        } else {
     ...
     */
    public void truVe() throws RemoteException {
     ...
     */
    public void clearTblGioVe() throws RemoteException {
    ...
    public void hienThiDanhSachChuyenTau(String chieu) {
        ...
        if (listChuyenDi.isEmpty()) {
            lblSoChuyenHienThi.setText("Số chuyến: 0");
            return;
        }

        // Đếm số chuyến ở trạng thái HOAT_DONG
        long soChuyenHienThi = 0;
        ...
        if (chieu.equalsIgnoreCase(ChieuChuyenTau.CHIEU_DI.getValue())) {
            ...
            for (ChuyenTau chuyenTau : listChuyenDi) {
                if (chuyenTau.getTrangThai().equals(ETrangThaiChuyenTau.HOAT_DONG.getTrangThai())) {
                    ...
                    soChuyenHienThi++; // Tăng số lượng hiển thị
                }
            }
            ...
            for (ChuyenTau chuyenTau : listChuyenVe) {
                if (chuyenTau.getTrangThai().equals(ETrangThaiChuyenTau.HOAT_DONG.getTrangThai())) {
                    JPanel btnChuyenTau = null;
                    try {
                        btnChuyenTau = getPnlChuyenTau(chuyenTau);
            ...
            lblSoChuyenHienThi.setText("Số chuyến: " + soChuyenHienThi);
        }
...
//    }
    public void hienThiDanhSachChoNgoi() throws RemoteException {
        if (toaDangChon == null || chuyenDangChon == null) {
            return;
        }

        // 1. Cập nhật danh sách vé đã bán cho chuyến hiện tại
        ArrayList<Ve> dsVeChuyen = daoVe.layDSVeDaBanTheoMaChuyen(chuyenDangChon.getMaChuyen());

        // Cập nhật listVeDaBan: xóa các vé cũ của chuyến này và thêm vé mới
        listVeDaBan.removeIf(v -> v.getChuyenTau().getMaChuyen().equals(chuyenDangChon.getMaChuyen()));
        listVeDaBan.addAll(dsVeChuyen);

        // 2. Lấy danh sách chỗ ngồi từ database
        listChoNgoi = daoChoNgoi.getDSChoNgoiTheoToa(toaDangChon.getMaToa());

        // 3. Xóa và cập nhật lại giao diện
        pnlDanhSachChoNgoi.removeAll();
        pnlDanhSachChoNgoi.setBorder(BorderFactory.createCompoundBorder(
                BorderFactory.createLineBorder(Color.BLACK),
                new EmptyBorder(10, 10, 10, 10)
        ...
        // 4. Hiển thị theo loại toa
        if (loaiToa.getTenLT().equals(ELoaiToa.NC.getLoaiToa()) ||
                loaiToa.getTenLT().equals(ELoaiToa.NM.getLoaiToa()) ||
                loaiToa.getTenLT().equals(ELoaiToa.NCL.getLoaiToa()) ||
                loaiToa.getTenLT().equals(ELoaiToa.NML.getLoaiToa())) {

            // Xử lý cho loại ghế ngồi
            for (int i = 0; i < 15; i++) {
                ArrayList<ChoNgoi> hang = new ArrayList<>();
                for (int j = 0; j < 4; j++) {
                    hang.add(listChoNgoi.get(i * 4 + j));
            ...
            for (int i = 0; i < mangHaiChieu.size(); i++) {
                ...
                hangChoNgoi.forEach(choNgoi -> {
                    int index = listChoNgoi.indexOf(choNgoi);
                    boxHangChoNgoi.add(getPnlChoNgoi(choNgoi, index));
                    boxHangChoNgoi.add(Box.createVerticalStrut(
                            hangChoNgoi.indexOf(choNgoi) == 1 ? 20 : 10
                    ));
                });
            ...
            hienThiChoNgoiLoaiGiuongNam(mangHaiChieu, 2, 15, 30);
        }
        else if (loaiToa.getTenLT().equals(ELoaiToa.BNL.getLoaiToa())) {
            hienThiChoNgoiLoaiGiuongNam(mangHaiChieu, 3, 13, 40);
        }
        else if (loaiToa.getTenLT().equals(ELoaiToa.VIP.getLoaiToa())) {
            hienThiChoNgoiLoaiVIP(mangHaiChieu, 2, 5, 10);
        }
...
//        // THÊM 2 DÒNG NÀY NGAY SAU KHI LẤY DANH SÁCH CHUYẾN ĐI
        updateAllBookedTickets(); // Cập nhật toàn bộ vé đã bán
        registerSeatListenerForCurrentTrain(); // Đăng ký listener cho chuyến tàu
    ...
    public void xinVe() throws RemoteException {
        ...
        //version1
        // Lấy danh sách mã ghế cần khóa
        List<String> seatIds = new ArrayList<>();
        seatIds.addAll(listChoNgoiDaChonChieuDi.stream()
                .map(ChoNgoi::getMaCho)
                .collect(Collectors.toList()));

        if (loaiVe.equalsIgnoreCase(ELoaiVe.KHU_HOI.getValue())) {
            seatIds.addAll(listChoNgoiDaChonChieuVe.stream()
                    .map(ChoNgoi::getMaCho)
                    .collect(Collectors.toList()));
        }

        String userId = nhanVien.getMaNV();
        String trainId = chuyenDangChon.getMaChuyen();
        //version2
        if (!daoVe.lockSeatsForBooking(trainId, seatIds, userId)) {
            System.out.println("Failed to lock seats");
            JOptionPane.showMessageDialog(null,
                    "Một số ghế đã được người khác chọn. Vui lòng chọn ghế khác.");
            return;
        }
        ...
        if (loaiVe.equalsIgnoreCase(ELoaiVe.MOT_CHIEU.getValue())) {
            System.out.println(chuyenDangChonChieuDi+"Chuyen dang chon neeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
            // Chọn vé một chiều
            JdXinVe jdXinVe = new JdXinVe(this, nhanVien, chuyenDangChonChieuDi, listChoNgoiDaChonChieuDi);
            jdXinVe.addWindowListener(new WindowAdapter() {
                @Override
                public void windowClosing(WindowEvent e) {
                    // Khi đóng cửa sổ, trả lại trạng thái ghế
//                    unlockAndResetSeats(trainId, seatIds, userId);
                    try {
                        for (String seatId : seatIds) {
                            unlockAndResetSeats(trainId, seatIds, userId);
                            daoVe.updateSeatStatus(seatId, ETrangThaiChoNgoi.CON_TRONG.getTrangThai());
                        }
                        //đổi remote thành ex
                    } catch (Exception ex) {
                        ex.printStackTrace();
    ...
    private void unlockAndResetSeats(String trainId, List<String> seatIds, String userId) {
        try {
            for (String seatId : seatIds) {
                daoVe.unlockSeat(trainId, seatId, userId);
                daoVe.updateSeatStatus(seatId, ETrangThaiChoNgoi.CON_TRONG.getTrangThai());
            }
            hienThiDanhSachChoNgoi();
        } catch (Exception ex) {
    ...
    @Override
    public void onUpdateChuyenTau(ChuyenTau chuyenTau) throws RemoteException {
        SwingUtilities.invokeLater(() -> {
            try {
                boolean isInCurrentList = listChuyenDi.stream()
                        .anyMatch(ct -> ct.getMaChuyen().equals(chuyenTau.getMaChuyen()));

                if (isInCurrentList) {
                    for (int i = 0; i < listChuyenDi.size(); i++) {
                        if (listChuyenDi.get(i).getMaChuyen().equals(chuyenTau.getMaChuyen())) {
                            listChuyenDi.set(i, chuyenTau);
                            break;
                        }
                    }

                    listVeDaBan.removeIf(ve -> ve.getChuyenTau().getMaChuyen().equals(chuyenTau.getMaChuyen()));
                    listVeDaBan.addAll(daoVe.layDSVeDaBanTheoMaChuyen(chuyenTau.getMaChuyen()));

                    if (chuyenDangChon != null && chuyenDangChon.getMaChuyen().equals(chuyenTau.getMaChuyen())) {
                        if (chuyenTau.getTrangThai().equals(ETrangThaiChuyenTau.DA_DUNG.getTrangThai())) {
                            JOptionPane.showMessageDialog(this,
                                    "Chuyến tàu " + chuyenTau.getMaChuyen() + " đã bị hủy. Vui lòng chọn chuyến khác.");
                            resetSelectedTrain();
                        } else {
                            chuyenDangChon = chuyenTau;
                            tauDangChon = chuyenDangChon.getTau();
                    ...
                    // Nếu chuyến mới được thêm vào -> load lại toàn bộ danh sách
                    listChuyenDi = daoChuyenTau.getDanhSachChuyenTauSapKhoiHanh();
                    hienThiDanhSachChuyenTau("Chiều đi");
                }
            } catch (RemoteException ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(this, "Lỗi kết nối khi cập nhật thông tin chuyến tàu");
            }
        });
    }
    private void registerSeatListenerForCurrentTrain() {
        try {
            // Hủy đăng ký listener cũ trước khi đăng ký mới
            if (nhanVien != null && nhanVien.getTaiKhoan() != null) {
                daoVe.unregisterSeatUpdateListener(nhanVien.getTaiKhoan().getMaTK());
            }
    ...
    private void updateAllBookedTickets() throws RemoteException {
        listVeDaBan.clear();
        // Cập nhật vé đã bán cho tất cả chuyến đi
        for (ChuyenTau chuyen : listChuyenDi) {
            listVeDaBan.addAll(daoVe.layDSVeDaBanTheoMaChuyen(chuyen.getMaChuyen()));
        }
        // Cập nhật vé đã bán cho tất cả chuyến về (nếu có)
        if (listChuyenVe != null) {
            for (ChuyenTau chuyen : listChuyenVe) {
                listVeDaBan.addAll(daoVe.layDSVeDaBanTheoMaChuyen(chuyen.getMaChuyen()));
            }
        }
    }
    // Trong PnlBanVe.java - Thêm log trong phương thức callback
    @Override
    public void onSeatStatusChanged(Ve ve) throws RemoteException {
        System.out.println("[CLIENT] Nhận thông báo thay đổi vé: " + ve.getMaVe());
        SwingUtilities.invokeLater(() -> {
            try {
                // Cập nhật toàn bộ danh sách vé đã bán cho chuyến tàu này
                listVeDaBan.removeIf(v ->
                        v.getChuyenTau().getMaChuyen().equals(ve.getChuyenTau().getMaChuyen()));
                listVeDaBan.addAll(daoVe.layDSVeDaBanTheoMaChuyen(ve.getChuyenTau().getMaChuyen()));

                // Nếu đang xem chuyến tàu có vé thay đổi
                if (chuyenDangChon != null &&
                        chuyenDangChon.getMaChuyen().equals(ve.getChuyenTau().getMaChuyen())) {
                    System.out.println("[CLIENT] Cập nhật UI cho chuyến " + chuyenDangChon.getMaChuyen());

                    // Cập nhật cả danh sách chỗ ngồi
                    hienThiDanhSachChoNgoi();

                    // Cập nhật lại thông tin toa
                    hienThiDanhSachToa();
                    hienThiThongTinToa();
                }
            } catch (Exception ex) {
                ex.printStackTrace();
                JOptionPane.showMessageDialog(this,
                        "Lỗi khi cập nhật trạng thái ghế: " + ex.getMessage());
            }
        });
    }

    // Thay thế phương thức finalize bằng phương thức cleanup mới
    public void cleanup() {
        try {
            if (daoVe != null && nhanVien != null && nhanVien.getTaiKhoan() != null) {
                daoVe.unregisterSeatUpdateListener(nhanVien.getTaiKhoan().getMaTK());
            }
            if (daoChuyenTau != null && nhanVien != null && nhanVien.getTaiKhoan() != null) {
                daoChuyenTau.unregisterUpdateChuyenTauListener(nhanVien.getTaiKhoan().getMaTK());
            }
            UnicastRemoteObject.unexportObject(this, true); // Thêm dòng này
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void resetSelectedTrain() {
        chuyenDangChon = null;
        tauDangChon = null;
        toaDangChon = null;
        listChoNgoiDaChonChieuDi.clear();
